(() => {
 ...
 if(/\/winchguard(?:\.html)?\/?$/.test(location.pathname)){
  const hero=document.querySelector('.case-visual');
  hero.outerHTML=figure('winchguard-presentation','아이디어에서, 장관상까지.','2026.09.15 · 해양수산 창업콘테스트 결선 발표 현장 · 아이디어 학생부 최우수상','feature-photo presentation-photo','jpg');
 ...
 if(/\/logistics(?:\.html)?\/?$/.test(location.pathname)){
