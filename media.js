(() => {
 ...
 if(location.pathname.endsWith('/winchguard.html')){
  ...
  afterHeading('시제품 검증 결과',`<figure class="evidence test-video"><video controls playsinline preload="metadata" aria-label="WinchGuard 시제품 시험 영상" aria-describedby="test-video-caption"><source src="${base}winchguard-test.mp4" type="video/mp4">이 브라우저에서는 영상을 재생할 수 없습니다. <a href="${base}winchguard-test.mp4">영상 파일 열기</a></video><figcaption id="test-video-caption"><strong>통제된 시험 환경에서 검증한 결과</strong><span>시제품 시험 영상 · 시험 조건과 측정 범위는 아래 설명 참조</span></figcaption></figure>`);
